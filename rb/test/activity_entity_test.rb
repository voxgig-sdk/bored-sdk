...
class ActivityEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = activity_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "activity." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BORED_TEST_ACTIVITY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    activity_ref01_data = nil
    if activity_ref01_data_raw.length > 0
...
def activity_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "activity", "ActivityTestData.json")
  ...
  entid_env_raw = ENV["BORED_TEST_ACTIVITY_ENTID"]
  ...
  if env["BORED_TEST_LIVE"] == "TRUE"
    ...
    client = BoredSDK.new(Helpers.to_map(merged_opts))
  end
