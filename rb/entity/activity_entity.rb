...
class ActivityEntity
  ...
  def make
    opts = @_entopts.dup
    ActivityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BoredHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Activity, Hash] the current Activity data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BoredHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Activity fields)
  def match_get
  ...
  #
  # @param reqmatch [ActivityLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Activity.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Activity, Hash] the loaded Activity; raises BoredError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BoredHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
