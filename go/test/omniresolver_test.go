...
import (
	...
	omni "github.com/voxgig-sdk/bored-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BoredError); is {
				out := map[string]any{
					"name":    "BoredError",
...
func omniCtx(arg any, client *sdk.BoredSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BoredSDK); is {
			client = live
