...
// Bored SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class BoredMakeError
{
    public static function call(?BoredContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BoredContext([], null);
        }
        $op = $ctx->op ?? new BoredOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BoredResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BoredError) ? $err->msg : (string)$err;
        $msg = "BoredSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new BoredError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof BoredError) {
