...
class FtCtrl extends BoredControl
{
...
class FtClient
{
    ...
    private BoredUtility $utility;

    public function __construct(BoredUtility $utility, array $options)
    {
    ...
    public function get_utility(): BoredUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BoredContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BoredContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BoredConfig::make_feature($name);
    ...
    private static function buildUrl(BoredSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BoredError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BoredResponse($fetched);
        }
    ...
    private function fail(BoredContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BoredContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
