...
{
    private static function utility(): BoredUtility
    {
        return new BoredUtility();
    }
    ...
    private static function ctx(array $over = []): BoredContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BoredContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BoredOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BoredError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BoredOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BoredMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BoredOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BoredMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BoredOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BoredMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BoredSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BoredMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BoredResponse([]),
            'result' => new BoredResult([]),
        ]);
        [, $err] = BoredMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BoredSpec(['step' => 's']),
            'response' => new BoredResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BoredResult([]),
        ]);
        [, $err] = BoredMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BoredSpec(['step' => 's']),
            'response' => new BoredResponse(self::resp(200, ['v' => 1])),
            'result' => new BoredResult([]),
        ]);
        [, $err] = BoredMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BoredSpec(['step' => 's']),
            'response' => new BoredResponse(self::resp(200, ['v' => 2])),
            'result' => new BoredResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BoredMakeResponse::call($ctx);
    ...
    {
        $preset = new BoredResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BoredSpec([]),
            'response' => new BoredResponse([]),
            'result' => new BoredResult([]),
        ...
        [$got, $err] = BoredMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BoredResult([])]);
        [, $err] = BoredMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BoredSpec([]), 'result' => null]);
        [, $err] = BoredMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BoredSpec(['step' => 's']),
            'result' => new BoredResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BoredOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BoredMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BoredSpec(['step' => 's']),
            'result' => new BoredResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BoredOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BoredMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BoredResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BoredSpec([]),
            'result' => new BoredResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BoredMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new BoredError('boom', 'boom');
        $utility->fetcher = function (BoredContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BoredSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BoredMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BoredContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BoredMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BoredResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BoredContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BoredSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BoredMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BoredResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BoredSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BoredMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BoredMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BoredMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BoredResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BoredDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BoredResult(['ok' => false])]);
        $this->expectException(BoredError::class);
        BoredDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BoredResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BoredMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BoredBaseFeature();
        $b = new BoredBaseFeature();
        BoredFeatureAdd::call($ctx, $a);
        BoredFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BoredBaseFeature
    {
        $f = new BoredBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): BoredContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BoredSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new BoredResponse(['headers' => null]),
            'result' => new BoredResult([]),
        ]);
        BoredResultHeaders::call($ctx);
