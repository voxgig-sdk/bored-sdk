...
AQL-driven command-line client **and** interactive REPL for the Bored
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bored-cli)
make build
...
./bored-cli load 1 activity            # {id:1} shorthand
./bored-cli load '{id:1}' activity       # explicit match map

# 4. Override the API base URL for a single call
BORED_BASE=https://api.example.com ./bored-cli load 1 activity

# 5. No arguments -> interactive REPL
./bored-cli
bored> load 1 activity
bored> :quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bored-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bored-cli load 1 activity
   ```
...
```sh
./bored-cli load 1 activity          # scalar shorthand for {id:1}
./bored-cli load '{id:1}' activity     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `bored>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `activity`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bored-cli`. |
...
(`./bored-cli <expr>`) and interactively (the REPL), and expressions compose the
