// Command Bored-cli is a CLI + REPL for the Bored SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BORED_APIKEY (api key), BORED_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BORED_APIKEY carries the API key and
	// BORED_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BORED_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BORED_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBoredSDK(opts)
