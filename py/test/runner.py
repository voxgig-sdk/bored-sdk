# Bored SDK test runner

from __future__ import annotations
import os
import json

from bored_sdk.utility.voxgig_struct import voxgig_struct as vs


class BoredTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BoredTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BoredTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BoredTestRunner.getenv("BORED_TEST_LIVE")
        override = BoredTestRunner.getenv("BORED_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BoredTestRunner.getenv(key)
        ...
        explain = BoredTestRunner.getenv("BORED_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BORED_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BoredTestRunner._test_control is not None:
            return BoredTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BoredTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BoredTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BoredTestRunner.load_test_control()
...
def load_env_local():
    BoredTestRunner.load_env_local()


def env_override(m):
    return BoredTestRunner.env_override(m)


def entity_data(v):
    return BoredTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BoredTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BoredTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BoredTestRunner.load_test_control()


def live_client_options():
    return BoredTestRunner.live_client_options()


def live_delay_ms():
    return BoredTestRunner.live_delay_ms()
