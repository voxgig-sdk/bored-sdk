...
//     (\`new BoredSDK(...)\` / \`BoredSDK.test(...)\` become
//     \`BoredSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new BoredSDK(...)\` and any
// \`BoredSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BoredSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BoredSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
