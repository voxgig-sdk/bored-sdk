...
//    only - a typed BoredContext there would make every ctx
...
final class BoredOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BoredContext($clean, null);
    ...
    public static function err_from_map($m): ?BoredError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BoredError(is_string($code) ? $code : '', $msg);
    }
