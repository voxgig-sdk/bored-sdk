...
class BoredSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = BoredHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = BoredHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!BoredFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, BoredFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return BoredUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = BoredHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = BoredHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = BoredHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new BoredError($op . "_allow",
                "BoredSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = BoredHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new BoredError("graphql_error",
                "BoredSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
