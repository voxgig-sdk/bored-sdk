...
MCP server exposing the Bored SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `bored_list` | `entity`, optional `query` map | First page of records as JSON |
| `bored_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `bored_list` and
`bored_load` tools become available in new sessions.
