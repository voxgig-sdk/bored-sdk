...
[MCP](https://modelcontextprotocol.io) server exposing the Bored SDK as
two agent tools — `bored_list` and `bored_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bored-mcp)
make build

# 2. Provide credentials via the environment
export BORED_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bored \
  -- /absolute/path/to/bored-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bored-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bored_list: first page of records
{ "entity": "activity" }
{ "entity": "activity", "query": { } }

// bored_load: one record by id
{ "entity": "activity", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bored-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bored \
     -- "$PWD"/dist/*/bored-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bored_list` and `bored_load` tools now appear
   in new sessions. Ask the agent to *"list activity using bored"*
   and it calls `bored_list` with `{"entity":"activity"}`.
...
```sh
export BORED_APIKEY=sk_live_xxx            # API key
export BORED_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `bored_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bored_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BORED_APIKEY` | API key sent with every request. |
| `BORED_BASE` | Optional override of the API base URL. |
...
```sh
./bored-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bored_load","arguments":{"entity":"activity","query":{"id":1}}}}'
```
